...
class LinearRegression:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        """
        if self.include_intercept_:     # if necessary add the intercept
            X = np.c_[np.ones(len(X)), X]
        self.coefs_ = np.linalg.pinv(X) @ y     # coefs_ = wˆ = X†y
        self.fitted_ = True
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.include_intercept_:     # if necessary add the intercept
            X = np.c_[np.ones(len(X)), X]
        return X @ self.coefs_      # return Xw
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        return (np.sum((y - y_pred) ** 2)) / y_pred.size
